...
class OneCClient:
    ...
    async def _classify_all_prices(self) -> tuple[dict, dict]:
        ...
        for register in ("InformationRegister_ЦеныНоменклатуры", "InformationRegister_Цены"):
            # Try with $select first (faster). If it fails (wrong field names), retry without.
            path_sel = (
                f"odata/standard.odata/{register}"
                f"?$format=json&$top=50000"
                f"&$select=Номенклатура_Key,Цена,ВидЦены_Key,ВидЦен_Key,ТипЦен_Key"
            )
            path_all = f"odata/standard.odata/{register}?$format=json&$top=50000"
            ok, data = await self._request("GET", path_sel)
            if not ok or not isinstance(data, dict) or not data.get("value"):
                if ok and isinstance(data, dict) and not data.get("value"):
                    logger.debug(f"1C price register {register} empty")
                else:
                    logger.debug(f"1C price register {register} $select failed — retrying without $select")
                # Retry full fetch — handles configs where field names differ (e.g. ВидЦен_Key)
                ok, data = await self._request("GET", path_all)
                if not ok or not isinstance(data, dict) or not data.get("value"):
                    continue
                logger.warning(f"1C {register} fields (no-$select): {list(data['value'][0].keys())}")
            ...
            for item in data["value"]:
                ...
                price = (item.get("Цена") or item.get("ЦенаЗаЕдиницу")
                         ...
                         or item.get("ЦенаЗаЕдиницуИзмерения"))
                type_key = str(
                    item.get("ВидЦены_Key") or item.get("ВидЦен_Key")
                    or item.get("ТипЦен_Key") or ""
                ).strip("{}")
